//
//  DropboxRelocationArgDictionarySerializerTests.m
//  ObjectiveDropbox
//
//  Created by Михаил Мотыженков on 07.04.16.
//  Copyright © 2016 Mikhail Motyzhenkov. All rights reserved.
//

#import <XCTest/XCTest.h>
#import "DropboxRelocationArg+DictionarySerializer.h"

@interface DropboxRelocationArgDictionarySerializerTests : XCTestCase

@property (nonatomic) DropboxRelocationArg *arg;

@end

@implementation DropboxRelocationArgDictionarySerializerTests

- (void)setUp {
    [super setUp];
    // Put setup code here. This method is called before the invocation of each test method in the class.
}

- (void)tearDown {
    // Put teardown code here. This method is called after the invocation of each test method in the class.
    [super tearDown];
}

- (void)testDictionary {
    NSString *fromPath = @"/";
    NSString *toPath = @"//";
    
    self.arg = [[DropboxRelocationArg alloc] initWithFromPath:fromPath toPath:toPath];
    NSDictionary *dic = [self.arg dictionaryFromParams];
    
    NSDictionary *expectedDic = @{@"from_path": self.arg.fromPath, @"to_path": self.arg.toPath};
    XCTAssertEqualObjects(dic, expectedDic);
}


@end
